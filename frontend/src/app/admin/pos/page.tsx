  ...
  const productCards = products.map((product) => {
    ...
    return (
      <button
        ...
        className="group flex h-full flex-col overflow-hidden rounded-3xl border border-black/10 bg-white text-left shadow-[0_12px_30px_-26px_rgba(0,0,0,0.55)] transition duration-300 hover:-translate-y-0.5 hover:border-black/20 hover:shadow-[0_18px_40px_-26px_rgba(0,0,0,0.55)]"
      >
        <div className="relative h-44 overflow-hidden bg-linear-to-br from-amber-50 via-white to-cyan-50">
          {product.image ? (
            <Image
        ...
        <div className="flex flex-1 flex-col gap-3 p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-black/55">{formatMoney(product.price)}</p>
            </div>
            <span
          ...
          <div className="mt-auto flex items-center justify-between gap-3">
            ...
            <div className="rounded-full bg-black px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition group-hover:bg-black/90">
              Add
            </div>
          </div>
        </div>
      ...
      <div className="space-y-8">
        <section className="relative overflow-hidden rounded-3xl border border-black/10 bg-[radial-gradient(circle_at_top_left,rgba(251,191,36,0.18),transparent_35%),radial-gradient(circle_at_top_right,rgba(34,211,238,0.16),transparent_30%),linear-gradient(180deg,#ffffff_0%,#fafaf9_100%)] p-6">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid w-full gap-3 sm:grid-cols-3 lg:max-w-105">
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_420px]">
          <div className="rounded-3xl border border-black/10 bg-white p-5 shadow-[0_16px_40px_-28px_rgba(0,0,0,0.45)]">
            ...
            <div className="mt-5 flex items-center justify-between text-sm text-black/55">
              <div className="flex w-full flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <span>{loadingProducts ? "Loading products..." : `${products.length} product${products.length === 1 ? "" : "s"} available`}</span>
                <span className="wrap-break-word text-right sm:max-w-[50%]">{deferredQuery ? `Searching for “${deferredQuery}”` : "Showing all active products"}</span>
              </div>
            </div>
              ...
              {loadingProducts ? (
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {Array.from({ length: 6 }).map((_, index) => (
                    <div key={index} className="h-72 rounded-3xl border border-black/10 bg-black/2 animate-pulse" />
                  ))}
                </div>
              ) : products.length > 0 ? (
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{productCards}</div>
              ) : (
                <div className="rounded-3xl border border-dashed border-black/15 bg-black/1.5 px-6 py-14 text-center">
                  ...
                  <p className="mt-2 text-sm text-black/55">
                    Try a different search term or clear the search box to view the full catalog.
                  </p>
                </div>
              )}
            </div>
          ...
          <aside className="rounded-3xl border border-black/10 bg-white p-5 shadow-[0_16px_40px_-28px_rgba(0,0,0,0.45)]">
